...
[出力フォーマット — この見出しと順番を厳守。装飾用の絵文字や記号は付けない]

## 今のあなた
あなた自身の性質を、相手が「これ自分のことだ」と感じる具体的な描写で2〜3段落（合計250〜400字）。「${shuku.catchy}」というタイプ呼称は本文に1回まで使ってよい。
ここでは性格・思考癖・対人傾向の素描だけを書く。今日の状況・行動アドバイス・気をつけたいクセは書かない（後のセクションで扱う）。

## 今日の流れ
今この瞬間に起きている内側の感覚と、空気の質を1〜2段落（150〜250字）。
ここは"観察"のみ。「〜するとよい」「〜してください」のような行動指示は書かない。

## 仕事と人間関係
仕事や日常の選び方、人との距離感、恋愛・友人関係での向き合い方を1〜2段落（200〜300字）。
ここではシチュエーション別の具体的な選び方だけを書く。「今のあなた」「今日の流れ」と同じ表現を繰り返さない。

## 今日やるとよい3つのこと
- 行動1（具体的に1〜2文で）
- 行動2（具体的に1〜2文で）
- 行動3（具体的に1〜2文で）
3つは互いに別カテゴリ（例: 環境を整える / 人に伝える / 自分に問う）にして、似た内容が並ばないようにする。「${shuku.caution}」のニュアンスは、3つのうち1つにだけさりげなく織り込む。

## 今日のキーフレーズ
短い1文（40字以内、句点1つ）で締める。前のセクションで使った言葉をそのまま繰り返さない、新しい比喩や視点で。

[厳守事項]
- 全体で 900〜1400 字程度。簡潔でリズム良く。
- 見出しは上記5つだけ。「序章」「はじめに」「まとめ」「終わりに」「気をつけたいクセ」「注意点」などを追加しない。
- セクション間で同じことを言い直さない。「あなたは○○な人」を別の言葉で言い換えるだけのループは禁止。各セクションは独立した役割を持つ。
- マークダウン装飾（\`**太字**\` \`*斜体*\` \`__\` \`~~\`）は一切使わない。見出しの「## 」と箇条書きの「- 」のみ使ってよい。
- カードのメッセージは、技法名や"カード"という語を使わず、「今日のあなたへ届いているサイン」「今の流れ」「内側で起きている変化」のように溶け込ませる。
- 禁止語（絶対に出さない）: 宿曜、二十七宿、○○宿（昴宿/畢宿など）、タロット、大アルカナ、正位置、逆位置。
- 末尾に「※エンタメ目的です」のような注釈は入れない（画面外で別途表示する）。
`
    }
  ];
...
人から強く言われて動くというより、自分の中で納得できた瞬間に集中力が一気に出るタイプ。周囲がまだ雰囲気で話している段階でも、あなたは細部の矛盾や、相手の本音、物事の弱い部分に先に気づいていることが多いはず。

## 今日の流れ

${direction}が来ています。届いているサインの大筋は「${cardMeaning}」というニュアンス。空気が少しだけいつもより素直に動いているので、無理に方向を変えなくても、その流れに身を任せる感覚で十分です。

## 仕事と人間関係

選び方では、広げるより絞る日。アイデアを増やすよりも、目的に合わないものを削るほうが結果につながります。お金や買い物は、衝動より比較・保留が吉。

人との距離感では、相手の反応を読み切ろうとしないこと。今日は推理するより、自分の希望を短く出すほうが関係が動きます。「少し話せる？」「この日どう？」のように相手が答えやすい形にすると流れが軽くなります。

## 今日やるとよい3つのこと

- 気になっている小さな未完了をひとつだけ片づける。
- ${reversed ? "ノートかメモに、今のもやもやを2行だけ言葉にする" : "誰かに、いま一番ありがたいと思っていることを短く伝える"}。
- ${shuku.caution.replace(/。$/,"")}——その逆を、ほんの少しだけ意識して動く。
